...
public class EndScreen implements Screen {
  /**
   * Parameters of EndScreen.
   * game-instance of the game
   * batch-parameter used for drawing the background
   * backgrounds-an array of textures used to store the moving backgrounds
   * sky-background that is going to be static on the screen
   * backgroundOffsets-an array of float values that is telling the speed at which the backgrounds are moving
   * backgroundMaxScrollingSpeed-a float value that indicates the maximum value at which a background can move
   * font-stores the font which is going to be used for drawing the text on screen
   * sound-stores the music which is played when the screen comes on
   */
  ...
  /**
   * This method is responsible for drawing the layers by drawing one background after the other.
  ...
  private void drawBackground(float deltaTime) {
    ...
    for (int layer = 0; layer < backgrounds.length; ++layer) {
      ...
      batch.draw(backgrounds[layer], -backgroundOffsets[layer] + Gdx.graphics.getWidth(), 0, 
          Gdx.graphics.getWidth(), Gdx.graphics.getHeight());
    }
  }
