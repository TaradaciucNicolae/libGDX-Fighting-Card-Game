...
public class FightScreen implements Screen {
	...
	/**
	 * @param game - refrence to our game Stage = the stage on which we draw our objects
	 * @param stage - 
	 * @param sound - music
	...
	public FightScreen(GdxFightingGame agame) {
		...
		for (i = 0; i <= game.p1.getNrCards(); ++i)
			dragAndDrop.addTarget(new DragAndDrop.Target(m1.animation) {
				...
				@Override
				public void drop(com.badlogic.gdx.scenes.scene2d.utils.DragAndDrop.Source source, Payload payload,
						...
						if (!game.p1.ListaCardsInMana.get(i).table.isVisible()) {
							...
							if (!m1.alive) {
								...
								win.setBounds((Gdx.graphics.getWidth() - newWidth) / 2,
										(Gdx.graphics.getHeight() - newHeight) / 2, newWidth, newHeight); // Center on
																										  // screen.
								stage.addActor(win);
				...
				@Override
				public boolean drag(com.badlogic.gdx.scenes.scene2d.utils.DragAndDrop.Source source, Payload payload,
						float x, float y, int pointer) {
					// TODO Auto-generated method stub
					return true;
				}
		...
		button.addListener(new ChangeListener() {
			...
			public void changed(ChangeEvent event, Actor actor) {
				...
				if (!game.p1.alive) {
					...
					float newWidth = 600, newHeight = 500;
					lose.setBounds((Gdx.graphics.getWidth() - newWidth) / 2, (Gdx.graphics.getHeight() - newHeight) / 2,
					...
					public void dragStop(InputEvent event, float x, float y, int pointer, DragAndDrop.Payload payload,
						...
						for (i = 0; i <= game.p1.getNrCards(); ++i)
							if (target == null) {
								...
								System.out.println("I got clicked! bababbababauin" + i);
								((HorizontalGroup) game.p1.ListaCardsInMana.get(game.p1.getNrCards()).table
										.getUserObject())
