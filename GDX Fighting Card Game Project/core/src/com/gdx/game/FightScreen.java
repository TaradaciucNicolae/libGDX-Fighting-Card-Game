...
public class FightScreen implements Screen {
	...
	public FightScreen(GdxFightingGame agame) {
		...
		final Player p = new Player(); // crearea playerului, impreauna cu cea a pachetului trebuie sa fie mutate in
										// ecranul / butonul de play
		p.CrearePachet(); // dar le-am lasat aici pt ca aici era creat playerul initial si pt a nu crea
							// confuzii
		...
		ButtonEndTurn.addListener(new ChangeListener() {
			...
			//	System.out.println("vei primi random 1,2 sau 3 carti");
				...
				/*
				int card_out_nr = new Random().nextInt(p.ListaCardsTotal.size());
				p.ListaCardsInMana.add(p.ListaCardsTotal.get(card_out_nr));
				p.ListaCardsTotal.remove(card_out_nr);
				AfisareCartiPeEcran(p);
*/
