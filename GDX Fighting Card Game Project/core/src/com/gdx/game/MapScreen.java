...
public class MapScreen implements Screen {
  ...
  public void TableButtons() {
    for (int i = 0; i < Fields.length; ++i)
      for (int j = 0; j < Fields[0].length; ++j) {
        if (i % 2 == 0) {
          if (Fields[i][j] == 1) {
            Buttons[i][j] = new TextButton("" 
                + (randomNumber.nextInt((600 - 100) + 1) + 100), skin);
  ...
  /**
   * This is a listener that sets the buttons to be touchable on the current row
   * and then creates a new FightScreen. Also updates the score with the number on
   * the buttons.
   */
